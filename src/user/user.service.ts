...
export class UserService {
  ...
  async findById(id: string): Promise<User | null> {
    const user = await this.userModel.findByPk(id, {
      include: [{ model: UserProfile }, { model: UserAddress }],
    });
  ...
  async createOrUpdateProfile(
    ...
    if (profile) {
      ...
      await user.update({
        ...
        ...(profileData.zone ? { zone: profileData.zone } : {}),
      });
    }
  ...
  async updateProfileImage(
    ...
    if (profile) {
      ...
      throw new BadRequestException(
        'Please create a profile first before uploading an image',
      );
    }
  ...
  async updateAddress(
    userId: string,
    addressId: string,
    updateData: UpdateUserAddressDto,
  ): Promise<UserAddress> {
    // Find the address
